...
class SchoolData:
    ...
    @classvar data (numpy.ndarray): The reshaped school data.
    ...
    def __init__(self, data, school_df):
        ...
        self.school_names_ids = self.set_schools_and_ids(school_df)

    def set_schools_and_ids(self, dataframe):
        """
        Creates a dictionary mapping school names to their IDs/codes.
    ...
    def look_up_school(self, id_or_name):
        ...
        @return id_or_name (str or int): School name or school code.
        @return tuple: School name and school code.
        @raises ValueError: If the input is not a valid school name or code.
        """
        if id_or_name in self.school_names_ids:
            school_name = id_or_name
            school_code = self.school_names_ids[id_or_name]
            return school_name, school_code
        elif int(id_or_name) in self.school_names_ids.values():
            school_code = int(id_or_name)
            school_name = next(name for name, code in self.school_names_ids.items() if code == school_code)
            return school_name, school_code
        else:
    ...
    def get_school_data(self, school_code):
        ...
        @return numpy.ndarray: The data array for the specified school.
        """
...
class Statistics:
    ...
    def school_enrol_grade_means(school_data):
        ...
        @param school_data (numpy.ndarray): The 2D data array for a specific school over the years wwith each grade.
    ...
    def print_school_stats(school_data):
        ...
        @param school_data (numpy.ndarray): The 2D data array for a specific school over the years wwith each grade.
        @print 
        Mean enrollment for grade 10, grade 11, and grade 12.
        Highest and lowest enrollment for a single grade.
        Total enrollment for each year.
        Total ten year enrollment.
        Mean total enrollment.
        Median enrollment over 500.
        """
    ...
    def general_stats(data_obj):
        ...
        Total graduating class of 2022.
        Highest enrollment for a single grade.
        Lowest enrollment for a single grade.
        """
...
def main():
    try:
        ...
        data = np.array([year_2013, year_2014, year_2015, year_2016, year_2017,
        ...
        user_input = input("Enter a school name or school code: ")
